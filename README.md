<div align="center">

  <!-- Header Banner (Exact Off-White/Light Gray Background with Permanent Marker Font) -->
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:E5E0DC,100:003545&height=220&section=header&text=Abdulla%20B&fontSize=70&fontColor=003545&fontFamily=Permanent+Marker&animation=fadeIn&fontAlignY=38" width="100%" />

  <!-- Dynamic Typing Header using Permanent Marker Font -->
  <h3>
    <a href="https://git.io/typing-svg">
      <img src="https://readme-typing-svg.demolab.com?font=Permanent+Marker&size=24&pause=1000&color=38BDF8&center=true&vCenter=true&width=650&lines=Data+Scientist+%26+AI%2FML+Engineer;Computer+Vision+%26+Deep+Learning+Specialist;Full-Stack+AI+Systems+Developer" alt="Typing SVG" />
    </a>
  </h3>

  <!-- Top Technology Badges -->
  <p>
    <a href="https://www.python.org/"><img src="https://img.shields.io/badge/Python-3.10+-3776AB?style=for-the-badge&logo=python&logoColor=white" /></a>
    <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript"><img src="https://img.shields.io/badge/JavaScript-ES6+-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" /></a>
    <a href="https://flask.palletsprojects.com/"><img src="https://img.shields.io/badge/Flask-2.0+-000000?style=for-the-badge&logo=flask&logoColor=white" /></a>
    <a href="https://www.tensorflow.org/"><img src="https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white" /></a>
    <a href="https://keras.io/"><img src="https://img.shields.io/badge/Keras-D00000?style=for-the-badge&logo=keras&logoColor=white" /></a>
    <a href="https://opencv.org/"><img src="https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white" /></a>
    <a href="https://www.netlify.com/"><img src="https://img.shields.io/badge/Netlify-00C7B7?style=for-the-badge&logo=netlify&logoColor=white" /></a>
    <a href="https://render.com/"><img src="https://img.shields.io/badge/Render-Live_Demo-brightgreen?style=for-the-badge&logo=render" /></a>
  </p>

</div>

---

### 👨‍💻 About Me

Passionate **Data Scientist & AI/ML Engineer** dedicated to transforming complex data into actionable business intelligence and engineering end-to-end computer vision web applications.

---

## 🛠️ Complete Tech Stack

* **Languages:** Python, SQL (MySQL, PostgreSQL, SQLite), JavaScript (ES6+), HTML5/CSS3
* **Data Science & Analytics:** Pandas, NumPy, Scikit-Learn, SciPy, Statsmodels, LightGBM
* **Deep Learning & Computer Vision:** TensorFlow, Keras, OpenCV, YOLO (v8 / v9), Convolutional Autoencoders
* **Business Intelligence:** Power BI (DAX, Power Query), Advanced Microsoft Excel
* **Web & Cloud Deployment:** Flask REST API, React 18, Tailwind CSS, Render, Netlify, Git/GitHub

---

## 🚀 Featured AI & Computer Vision Projects

### ⚠️ Hazardous Object Detection System
* **Tech:** YOLOv9, Python, Flask, Render
* **Description:** Real-time AI computer vision web application detecting hazardous objects from uploaded images and live webcam streams.

### 🔧 WeldVision AI — Intelligent Weld Detection
* **Tech:** YOLO, OpenCV, Flask, React, Tailwind CSS
* **Description:** Automated weld quality inspection web application classifying joints (Good Weld, Bad Weld, Defect) with bounding box tracking, latency metrics, and live webcam capture.

### 🧠 MNIST AutoEncoder Web-App
* **Tech:** Convolutional Autoencoders, Keras/TensorFlow, Flask, React
* **Description:** Interactive deep learning laboratory for image denoising, metric evaluation, and real-time 2D latent space activation painting.

---

## 🗺️ Learning Journeys

| Repository | Focus Areas |
| :--- | :--- |
| **[🐍 Python-Journey](./Python-Journey)** | Fundamentals, Data Structures, OOP, Visualization, Mini-Projects |
| **[🗄️ SQL-Journey](./SQL-Journey)** | Complex Queries, Joins, Window Functions, CTEs, Aggregations |
| **[📊 Statistics-Journey](./Statistics-Journey)** | Hypothesis Testing, Z/T-Tests, ANOVA, Chi-Square, Inferential Stats |
| **[🍫 Excel-Journey](./Awesome-Chocolates-Sales-Analysis-Excel)** | Sales Dashboards, Advanced Formulas, Pivot Tables, Data Cleaning |
| **[📈 Power-BI-Journey](./Power-BI-Journey)** | Capstone Dashboards, DAX Measures, Data Modeling, Healthcare Analytics |
| **[🤖 Machine-Learning-Journey](./Machine-Learning-Journey)** | Preprocessing, Feature Engineering, Regression, Boosting, Clustering |
| **[🧠 Deep-Learning-Journey](./Deep-Learning-Journey)** | ANNs, CNNs, Optimization, Backpropagation, Model Evaluation |

---

## 📫 Connect With Me

* 📧 **Email:** [abdullfhariq.official@gmail.com](mailto:abdullfhariq.official@gmail.com)
* 🐙 **GitHub:** [@Abdulla-B-Official](https://github.com/Abdulla-B-Official)
